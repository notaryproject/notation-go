...
func Example_localVerify() {
	...
	// Users could use the default trust store from Notary. The trust store
	// spec: https://github.com/notaryproject/notaryproject/blob/v1.0.0-rc.1/specs/trust-store-trust-policy.md#trust-store
	createTrustStore()
...
-----END CERTIFICATE-----`
	...
	os.MkdirAll("tmp/truststore/x509/ca/valid-trust-store", 0700)
	pemCertFile, err := os.Create("tmp/truststore/x509/ca/valid-trust-store/NotationExample.pem")
	if err != nil {
		panic(err)
	}
	err = pem.Encode(pemCertFile, block)
	if err != nil {
		panic(err)
	}
	pemCertFile.Close()
