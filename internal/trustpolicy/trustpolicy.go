...
// Document represents a common trust policy document model that can
// support multiple trust policy versions
//
// All the trust policy versions will be converted to this model before being
...
func (trustPolicyDoc *Document) GetApplicableTrustPolicy(artifactReference string) (*TrustPolicy, error) {
	...
	for _, policyStatement := range trustPolicyDoc.TrustPolicies {
		if slices.Contains(policyStatement.RegistryScopes, Wildcard) {
...
func (policyDoc *Document) Validate() error {
	...
	for _, statement := range policyDoc.TrustPolicies {
		...
		if verificationLevel.Name == "skip" {
			if !statement.TrustStores.IsEmpty() || !statement.TrustedIdentities.IsEmpty() {
...
type TrustPolicy struct {
	...
	TrustedIdentities TrustIdentities
}

// clone returns a deep copy of the TrustPolicy
func (tp *TrustPolicy) clone() *TrustPolicy {
	// Helper function to copy string slices
	...
	// Helper function to copy map[ValidationType]ValidationAction
	copyMap := func(src map[ValidationType]ValidationAction) map[ValidationType]ValidationAction {
		dst := make(map[ValidationType]ValidationAction)
		for k, v := range src {
			dst[k] = v
		}
		return dst
	...
	return &TrustPolicy{
		Name:           tp.Name,
		RegistryScopes: copySlice(tp.RegistryScopes),
		SignatureVerification: SignatureVerification{
			VerificationLevel: tp.SignatureVerification.VerificationLevel,
			Override:          copyMap(tp.SignatureVerification.Override),
		},
		TrustStores: TrustStores{
			CA:               copySlice(tp.TrustStores.CA),
			SigningAuthority: copySlice(tp.TrustStores.SigningAuthority),
		},
		TrustedIdentities: TrustIdentities{
			CA:               copySlice(tp.TrustedIdentities.CA),
			SigningAuthority: copySlice(tp.TrustedIdentities.SigningAuthority),
		},
	}
}
...
func (ts *TrustStores) IsEmpty() bool {
	return len(ts.CA) == 0 && len(ts.SigningAuthority) == 0
}

type TrustIdentities struct {
	CA               []string
	SigningAuthority []string
}

func (ti *TrustIdentities) IsEmpty() bool {
	return len(ti.CA) == 0 && len(ti.SigningAuthority) == 0
}
...
func validateTrustStore(statement TrustPolicy) error {
	for _, namedStore := range statement.TrustStores.CA {
		if !file.IsValidFileName(namedStore) {
			return fmt.Errorf("trust policy statement %q uses an unsupported trust store name %q. Named store name needs to follow [a-zA-Z0-9_.-]+ format", statement.Name, namedStore)
		}
	}

	for _, namedStore := range statement.TrustStores.SigningAuthority {
		if !file.IsValidFileName(namedStore) {
			return fmt.Errorf("trust policy statement %q uses an unsupported trust store name %q. Named store name needs to follow [a-zA-Z0-9_.-]+ format", statement.Name, namedStore)
		}
	}
...
func validateTrustedIdentities(statement TrustPolicy) error {
	if err := validateTrustedIdentitiesDetails(statement.Name, statement.TrustedIdentities.CA); err != nil {
		return err
	}

	if err := validateTrustedIdentitiesDetails(statement.Name, statement.TrustedIdentities.SigningAuthority); err != nil {
		return err
	}
...
func validateTrustedIdentitiesDetails(policyName string, trustedIdentities []string) error {
	// If there is a wildcard in trusted identies, there shouldn't be any other
	//identities
	if len(trustedIdentities) > 1 && slices.Contains(trustedIdentities, Wildcard) {
...
func validateRegistryScopes(policyDoc *Document) error {
	...
	for _, statement := range policyDoc.TrustPolicies {
		...
		if len(statement.RegistryScopes) > 1 && slices.Contains(statement.RegistryScopes, Wildcard) {
