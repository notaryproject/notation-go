...
import (
	...
	fplugin "github.com/notaryproject/notation-plugin-framework-go/plugin"
...
func (v *verifier) processSignature(ctx context.Context, sigBlob []byte, envelopeMediaType string, trustPolicy *trustpolicy.TrustPolicy, pluginConfig map[string]string, outcome *notation.VerificationOutcome) error {
	...
	var pluginCapabilities []fplugin.Capability
	...
	var installedPlugin fplugin.VerifyPlugin
...
func executePlugin(ctx context.Context, installedPlugin fplugin.VerifyPlugin, trustPolicy *trustpolicy.TrustPolicy, capabilitiesToVerify []pluginframework.Capability, envelopeContent *signature.EnvelopeContent, pluginConfig map[string]string) (*pluginframework.VerifySignatureResponse, error) {
	logger := log.GetLogger(ctx)
	// sanity check
	if installedPlugin == nil {
