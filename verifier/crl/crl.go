...
func (c *FileCache) Get(ctx context.Context, url string) (*corecrl.Bundle, error) {
	...
	if err != nil {
		...
		logger.Debugf("failed to get crl bundle from file cache with key %q: %s", url, err)
	...
	if err := json.Unmarshal(contentBytes, &content); err != nil {
		logger.Debugf("failed to decode file retrieved from file cache: %s", err)
	...
	if err != nil {
		logger.Debugf("failed to parse base CRL of file retrieved from file cache: %s", err)
	...
	if content.DeltaCRL != nil {
		...
		if err != nil {
			logger.Debugf("failed to parse delta CRL of file retrieved from file cache: %s", err)
	...
	if err := checkExpiry(ctx, bundle.BaseCRL.NextUpdate); err != nil {
		logger.Debugf("check BaseCRL expiry failed: %s", err)
		return nil, err
	}
	if bundle.DeltaCRL != nil {
		if err := checkExpiry(ctx, bundle.DeltaCRL.NextUpdate); err != nil {
			logger.Debugf("check DeltaCRL expiry failed: %s", err)
			return nil, err
		}
	}
...
func (c *FileCache) Set(ctx context.Context, url string, bundle *corecrl.Bundle) error {
	...
	if err != nil {
		logger.Debugf("failed to store crl bundle in file cache: %s", err)
